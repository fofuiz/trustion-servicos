...
public class RelatorioAnaliticoCreditoD1Specification {
    ...
    private RelatorioAnaliticoCreditoD1Specification() {
        throw new IllegalAccessError("Specification class");
    }

    public static Specification<RelatorioAnaliticoCreditoD1> byCriterio(RelatorioAnaliticoCreditoD1DTO dto, List<Empresa> empresasPermissao, List<GrupoEconomico> gruposPermissao) {
        return (Root<RelatorioAnaliticoCreditoD1> root, CriteriaQuery<?> query, CriteriaBuilder builder) -> {
            ...
            if (dto != null) {
                ...
                equal(builder, root, predicates, "cnpj", dto.getCnpj());
                equal(builder, root, predicates, "gtv", dto.getGtv());
                ...
                greaterThanOrEqualTo(builder, root, predicates, DATA_COLETA, dto.getDataInicial());
                lessThanOrEqualTo(builder, root, predicates, DATA_COLETA, dto.getDataFinal());

            }

            if (isNotEmpty(empresasPermissao)) {
                List<Long> idsEmpresas = new ArrayList<>();
                empresasPermissao.forEach(emp -> idsEmpresas.add(emp.getIdEmpresa()));
            ...
            if (isNotEmpty(dto.getOutrasEmpresas())) {
                ...
                gruposPermissao.forEach((grupo) -> idsGrupos.add(grupo.getIdGrupoEconomico()));
                in(builder, root, predicates, ID_GRUPO_ECONOMICO, idsGrupos);
            }

            query = query.orderBy(builder.asc(root.get(DATA_COLETA)));
